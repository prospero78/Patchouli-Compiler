...
PROCEDURE Identifier(VAR sym: INTEGER);
	VAR
		i, k2: INTEGER;
		bNotDigit, bNotSpez, bNotEngBig, bNotEngSmall, bNotRusBig, bNotRusSmall: BOOLEAN;
	BEGIN i := 0;
		REPEAT
			IF i <= MaxIdLen THEN
				id[i] := ch;
			...
			bNotDigit := (ch < '0') OR (ch > '9');
			bNotSpez :=(ch # '_');
			bNotEngBig := (ch < 'A') OR  (ch > 'Z');
			bNotEngSmall := (ch < 'a') OR (ch > 'z');
			bNotRusBig := (ch < 'А') OR  (ch > 'Я');
			bNotRusSmall := (ch < 'а') OR (ch > 'я');

			UNTIL  bNotDigit & bNotSpez &
					 bNotEngBig & bNotEngSmall &
					 bNotRusBig & bNotRusSmall;
